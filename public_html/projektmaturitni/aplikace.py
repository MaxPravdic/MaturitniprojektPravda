...
def check_date():
    ...
    cursor.execute("SELECT spz, model, username, barva FROM `1auta` WHERE datum_navstevy = %s", (date,))
    results = cursor.fetchall()
    db.close()

    if results:
        display_text = "\n".join([f"SPZ: {r['spz']}, Model: {r['model']}, Uživatel: {r['username']}, Barva: {r['barva']}" for r in results])
...
def show_upcoming_visits():
    db = connect_db()
    cursor = db.cursor(dictionary=True)
    cursor.execute("SELECT datum_navstevy, spz, model, username FROM `1auta` WHERE datum_navstevy >= CURDATE() ORDER BY datum_navstevy ASC")
    results = cursor.fetchall()
    db.close()

    if results:
        visits_text = "\n".join([f"{r['datum_navstevy']} - {r['spz']} ({r['model']}) - {r['username']}" for r in results])
...
# Načtení loga
logo_path = os.path.abspath("logo.png")  # Absolutní cesta k logu
if os.path.exists(logo_path):
    global logo_img  # Uložit jako globální proměnnou
    logo_img = PhotoImage(file=logo_path)
    resized_logo = logo_img.subsample(3, 3)  
    logo_label = tk.Label(root, image=resized_logo, bg="#2E2E2E")
    logo_label.pack(pady=(10, 5))  # Posunout logo nahoru
else:
